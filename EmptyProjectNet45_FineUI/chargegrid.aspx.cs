...
{
    public partial class chargegrid : System.Web.UI.Page
    {
        ...
        {
            SqlConnection conn = new SqlConnection(Connstring);
            string sqlStr = "update [charge] set licensenum='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[1].Controls[0])).Text.ToString().Trim() + "',approver='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[2].Controls[0])).Text.ToString().Trim() + "',date='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[3].Controls[0])).Text.ToString().Trim() + "',enter='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[4].Controls[0])).Text.ToString().Trim() + "',leave='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[5].Controls[0])).Text.ToString().Trim() + "',hour='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[6].Controls[0])).Text.ToString().Trim() + "',min='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[7].Controls[0])).Text.ToString().Trim() + "',state='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[8].Controls[0])).Text.ToString().Trim() + "',receivables='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[9].Controls[0])).Text.ToString().Trim() + "',Payment='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[10].Controls[0])).Text.ToString().Trim() + "' where ord='"
        ...
        {
            SqlConnection conn = new SqlConnection(Connstring);
            string sqlStr = "delete from [charge] where ord='" + GridView2.DataKeys[e.RowIndex]["ord"].ToString() + "'";
        ...
        protected void FINDBUTTOM_Click(object sender, EventArgs e)
        {
            ...
            if (key != "")
            {
                string sqlStr = "Select * from [charge] "; //模糊查询，%可以替代任意长度字符
                sqlStr += "where licensenum like '%" + key + "%' "; //按姓名搜索
                sqlStr += "or approver like '%" + key + "%' ";//按遗学号搜索
                sqlStr += "or state like '%" + key + "%' ";//按班级搜索
                sqlStr += "or date like '%" + key + "%' ";
                SqlConnection conn = new SqlConnection(Connstring);
                SqlDataAdapter adapter = new SqlDataAdapter(sqlStr, conn);
                DataSet ds = new DataSet();
                conn.Open();
                adapter.Fill(ds, "charge");

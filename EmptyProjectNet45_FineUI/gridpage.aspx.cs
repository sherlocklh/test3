...
namespace EmptyProjectNet45_FineUI
{
    public partial class gridpage : System.Web.UI.Page
    {
        ...
        {
            SqlConnection conn = new SqlConnection(Connstring);
            string sqlStr = "update [applylicense] set number='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[1].Controls[0])).Text.ToString().Trim() + "',name='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[2].Controls[0])).Text.ToString().Trim() + "',licensenum='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[3].Controls[0])).Text.ToString().Trim() + "',email='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[4].Controls[0])).Text.ToString().Trim() + "',approver='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[5].Controls[0])).Text.ToString().Trim() + "',date='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[6].Controls[0])).Text.ToString().Trim() + "',time='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[7].Controls[0])).Text.ToString().Trim() + "',describe='"
                + ((TextBox)(GridView2.Rows[e.RowIndex].Cells[8].Controls[0])).Text.ToString().Trim() + "' where ord='"
        ...
        protected void FINDBUTTOM_Click(object sender, EventArgs e)
        {
            ...
            if (key != "")
            {
                string sqlStr = "Select * from [applylicense] "; //模糊查询，%可以替代任意长度字符
                sqlStr += "where name like '%" + key + "%' "; //按姓名搜索
                sqlStr += "or licensenum like '%" + key + "%' ";//按遗学号搜索
                sqlStr += "or approver like '%" + key + "%' ";//按班级搜索
                sqlStr += "or date like '%" + key + "%' ";
                SqlConnection conn = new SqlConnection(Connstring);
                SqlDataAdapter adapter = new SqlDataAdapter(sqlStr, conn);
                DataSet ds = new DataSet();
                conn.Open();
                adapter.Fill(ds, "applylicense");
